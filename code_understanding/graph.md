# <font size=7>**graph.py**</font>


><font size=4>Graph file aim to create and update or reset the attributes(edge or node) in graph.</font>

# <font size=5>Input:</font>
<font size=4> Only **two inputs**, (n and m), are needed corresponds to the dimension of grid.
</font>

# <font size=4>Output:</font>
<font size=4>The **Output** is obviously the created grid.
</font>

# <font size=4>Process:</font>
<font size=4>The whole process is basiclly using the methods like: grid_2d_graph, nx.set_edge_attributes, imported from library, networkx, to intialise the parameters.
</font>

```mermaid
flowchart LR

    Start --> option1["Network(n,m)"]
    Start --> option2["reset_graph_state(G)"]
    Start --> option3["reset_graph_usage(G)"]
    Start --> option4["update_graph_usage(G, reps)"]
    Start --> option5["update_graph_params(G, p=None, Qc=None)"]
    Start --> option6["set_p_edge(G, p_op=0.8, loss_dB=None)"]
    Start --> option7["set_edge_length(G, length=1, p_op=0.8, loss_dB=0.2)"]
    Start --> option8["get_entangled_subgraph(G)"]
    Start --> option9["update_usage_from_subgraph(G, J)"]
    Start --> option10["remove_nodes(G: nx.Graph, min_usage, excluded_nodes=None)"]


    option1 --> aim1(initializes a 2D grid graph, setting default parameters)
    option2 --> aim2(initialize the link state)
    option3 --> aim3(initialize the usage parameters)
    option4 --> aim4(updates usage ratios)
    option5 --> aim5(sets probability 'p' and quantum capacity 'Qc' for edges/nodes)
    option6 --> aim6(modify edge properties based on signal loss)
    option7 --> aim7(modify edge properties based on signal loss)
    option8 --> aim8(creates a subgraph with entangled edges)
    option9 --> aim9( transfers usage data from a subgraph)
    option10 --> aim10(deletes nodes based on usage thresholds)
